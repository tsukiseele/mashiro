...
export default class Mashiro<T extends Meta> {
   ...
   * @param {Rules} site 规则
   */
  constructor(site: Rules) {
    this.site = site
  }
  setSite(site: Rules): Mashiro<T> {
    this.site = site
    return this
  }
  setPage(page: number): Mashiro<T> {
    this.page = page
    return this
  }
  setKeywords(keywords: string): Mashiro<T> {
  ...
  async parseRules(_url: string, props: Props, page: number = this.page, keywords: string | undefined = this.keywords): Promise<T[]> {
    ...
    // 发送请求
    // @ts-ignore
    const html = await this.requestText(url, { headers: this.site?.headers || this.site?.$site.headers })
    // 检查无效响应
    if (!html) return []
    // 加载文档
   ...
   * @returns {String} 结果
   */
  private replaceRegex(text: string, capture?: string, replacement?: string): string {
   ...
   * @returns
   */
  private objectEquals(x: any, y: any, deep: boolean = false): boolean {
    if (x === y) return true
